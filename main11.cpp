...
std::string get_absolute_path(std::string rel)
{
...
int main(int argc, char* argv[]) {
    ...
    std::string exe = wstring_to_utf8(Py_GetProgramFullPath());
    // get dir of the EXE
    std::string dir = fs::path(exe).parent_path().u8string();
    // Path to python
    std::string python = dir + "\\python37";
    // Build PyPath to the embedded zip file
    
    // string to the 'mod' script
    std:: string script = dir + "\\script.py";
   
    // Point to python dir and the python zip
    std::string pypath = dir + "\\python37\\python37.zip;"
     +python;
    ;
    
    Py_SetPath(utf8_to_wstring(pypath).c_str());
    Py_SetPythonHome(utf8_to_wstring(python).c_str());
    ...
    if (argc >= 2 && argv[1][0]=='r') {
        ...
        py::print("Waiiting for debugger to attach...");
